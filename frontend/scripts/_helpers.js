...
export async function findFiles(basePath, predicate, options={}) {
...
export async function compileSassAll(worker) {
  ...
  return result.reduce((acc, item, index) => {
    acc[item.outputPath] = Object.assign(item, {index});
    return acc;
  }, {});
}
...
export function concatSass(files) {
  const output = []

  const values = Object.values(files);
  values.sort((a, b) => compare(a.index, b.index));

  for (let item of values) {
    output.push(item.css);
  }
...
export async function readShadowManifest() {
  try {
...
export async function renderTemplate(path, context={}, partials={}) {
...
export async function generateSvgSprite(files, prefix) {
